...
2. 运行 `gitee skills list --json`，确认 gitee-pr、gitee-api、gitee-repo、gitee-search、gitee-issue 均为已安装状态。
...
本仓库提供 **5 个 Gitee CLI 技能**，覆盖官方 CLI 的核心能力面。每个技能都遵循同一套安全约定：

- 所有命令强制 `--no-tui`（禁用交互式 TUI，避免 Agent 阻塞）；
- 支持结构化输出的命令一律加 `--json`；
- **绝不使用 `--ai` 系列 flag**（会触发交互式 pager / 确认，导致阻塞）；
- 删除等**不可逆操作**在执行前必须向用户明确确认。
...
| 技能 | 作用 | 触发示例 |
|------|------|---------|
| **gitee-pr** | Pull Request 全生命周期：创建 / 列出 / 查看 / diff / 审查(approve) / 评论 / 合并 / 关闭 / 重开 / 拉到本地(checkout/fetch)。合并需确认。 | "提个 PR"、"看看 PR"、"review / 通过 PR"、"合并 PR"、"把 PR 拉到本地跑一下" |
...
- 每个技能是 `skills/<name>/SKILL.md` 单文件；
- frontmatter 必须包含 `name`、`description`（含中英文触发词）、`compatibility`、`metadata.author`、`metadata.version`；
